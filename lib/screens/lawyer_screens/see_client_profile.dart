...
class _SeeClientProfileState extends State<SeeClientProfile> {
  ...
  Widget build(BuildContext context) {
    final categoryList=Provider.of<MyProfileProvider>(context).profileData['lawyerProfile']['expertise'];
    return Scaffold(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Container(
                        ...
                        child: InkWell(
                          onTap: (){
                            Navigator.push(context, MaterialPageRoute(builder: (context)=>SeeJobs(client: widget.client, categoryList: categoryList)));},
                          child: const Center(
                            child: Text(
                              "Book",
                              style: TextStyle(
...
class _SeeJobsState extends State<SeeJobs> {
  ...
  Future<List<Map<String,dynamic>>> getJobsofTHatClient()async{
    try{
      QuerySnapshot getServiceSnapshot=await FirebaseFirestore.instance.collection('jobs').where('clientId',isEqualTo: widget.client['id']).where('status',isEqualTo: 'pending').where('category',whereIn: widget.categoryList).get();
     // print('${getServiceSnapshot.docs.map((doc)=>doc.data() as Map<String,dynamic>).toList()} ooooooooooooooo');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: FutureBuilder(future: getJobsofTHatClient(), builder: (context,snapshot){
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return FutureBuilder<bool>(
                      future: check.checkAlreadyJobBuy(widget.client['id'], job['jobId']),
        ...
        return StatefulBuilder(
          builder: (BuildContext context, StateSetter setState) {
            return SingleChildScrollView(
              child: AlertDialog(
                title: Text('Request Job for ${service["title"]}',maxLines: 4,overflow: TextOverflow.ellipsis,),
                content: Column(
                ...
                actions: [
                  ...
                  TextButton(
                    onPressed: () {
                      ...
                      if (requestDetails.isNotEmpty && priceDetails.isNotEmpty ) {
  ...
  void _submitRequest(Map<String, dynamic> job, String requestDetails,String priceDetails,) async {
